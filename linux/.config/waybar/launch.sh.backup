...
if [ ! -f $HOME/.config/ml4w/settings/waybar-disabled ]; then
    ...
    HYPRLAND_INSTANCE_SIGNATURE="$HYPRLAND_SIGNATURE" waybar -c ~/.config/waybar/themes${arrThemes[0]}/$config_file -s ~/.config/waybar/themes${arrThemes[1]}/$style_file >/dev/null 2>&1 &
    ...
    sleep 1
    sudo renice -n -5 -p $(pgrep waybar) 2>/dev/null
else
